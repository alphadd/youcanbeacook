...
<form action="<?php echo esc_url( get_permalink( wc_get_page_id( 'change_password' ) ) ); ?>" method="post">
	...
	<button class="button btn btn-default" name="change_password"><?php _e( 'Save', 'woocommerce' ); ?></button>

	<?php wp_nonce_field( 'woocommerce-change_password' ); ?>
	<input type="hidden" name="action" class="form-control" value="change_password" />
