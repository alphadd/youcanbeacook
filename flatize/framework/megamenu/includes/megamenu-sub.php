...
class PGL_Megamenu_Sub extends Walker_Nav_Menu {
   ...
   public  function start_lvl(  &$output, $depth = 0, $args = array(), $megaconfig=null ) {
		...
		$output	   .= "\n$indent<div class=\"{$class}\" $attrw ><div class=\"dropdown-menu-inner\"><div class=\"row\"><div class=\"pgl-col col-md-{$fcolswidth} {$colclass}\" {$cattrs} data-type=\"menu\"><div class=\"pgl-col-inner\"><ul class=\"megamenu-items\">\n";
	}
	 ...
	 */
	public function end_lvl( &$output, $depth = 0, $args = array(),$megaConfig=null ) {
	 ...
	 * @return String $output
	 */
	protected function genMegaMenuByConfig( $menu  ){
		...
		$output = '';
			...
			foreach( $menu->megaconfig->rows  as $row ){
				...
				$output .= '<div class="row">';
					foreach( $row->cols as $col ){
						$colclass = isset($col->colclass)?$col->colclass:'';
						 $output .= '<div class="pgl-col col-md-'.$col->colwidth.' '.$colclass.'" '.$this->getColumnDataConfig( $col ).'> <div class="pgl-col-inner">';
			...
			if( !empty($widgets) ){
				...
				foreach( $widgets as $wid ){
					...
					if( $o ){
						...
						$output .= '<div id="wid-'.$wid.'" class="pgl-widget">'.$content.'</div>';
					}
				}
				return $output;
			}
