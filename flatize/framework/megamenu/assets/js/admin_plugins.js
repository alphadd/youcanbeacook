var PGL_Admin = window.PGL_Admin || {};
!function ($) {
	"use strict";
	$.extend(PGL_Admin, {
		init: function(){
			$('#section-customize-theme,#section-megamenu-menu').find('.explain a').addClass('button-primary').css('float','none');
			PGL_Admin.PageOption();
		},
		//define
		// params
	    params_Embed : function (input_id,parent_id){
	    	...
	    	if(input.length>0){
		    	...
		    	if(oembed_url!="" && oembed_url.length>6){
		    		...
		    		PGL_Admin.param_start_ajax_embed(oembed_url,parent_id);
		    	}
		        input.on('keyup', function (event) {
					PGL_Admin.param_ajax_Embed(jQuery(this), event,parent_id,input_id);
				});
			}
	    },
		...
		// Page Option Config
		PageOption : function(){
	    	var check = jQuery('#pgl-config .page-layout select').val();
			jQuery('#pgl-config .page-layout img').each(function(index,item){
				if(check == jQuery(this).attr('data-value'))
					jQuery(this).addClass('selected');
			});
			jQuery('#pgl-config .page-layout img').on("page-layout-select",function(){
				jQuery('#pgl-config .page-layout img.layout').removeClass('selected');
				jQuery(this).addClass('selected');
				var arr = jQuery(this).attr('data-value').split('-');
				if(arr[0]=='1' || arr[0] == 'm'){
					jQuery('#pgl-config .left-sidebar').slideDown(400);
				}else{
					jQuery('#pgl-config .left-sidebar').slideUp(400);
				}
				if(arr[2]=='1' || arr[2] == 'm'){
					jQuery('#pgl-config .right-sidebar').slideDown(400);
				}else{
					jQuery('#pgl-config .right-sidebar').slideUp(400);
				}
			});
			jQuery('#pgl-config .page-layout img.selected').trigger('page-layout-select');
			jQuery('#pgl-config .page-layout img.layout').click(function(event) {
				jQuery(this).addClass('selected').trigger('page-layout-select');
				jQuery('#pgl-config .page-layout select').val(jQuery(this).attr('data-value'));
			});
	    }
	});
